    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6" enctype="multipart/form-data">
        ...
        <div>
            <label class="block mb-2 text-sm font-medium text-gray-800 dark:text-white" for="avatar">Upload
                avatar</label>
            <input
                @error('avatar') bg-red-50 border-red-500 text-red-900 placeholder-red-700 focus:ring-red-500 focus:border-red-500 @enderror
                class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400"
                aria-describedby="avatar_help" id="avatar" name="avatar" type="file"
                accept="image/png, image/jpg, image/jpeg">
            <div class="mt-1 text-sm text-gray-500 dark:text-gray-300" id="avatar_help">.png, jpg, or .jpeg</div>
            @error('avatar')
...
@push('script')
    ...
    <script>
        // register plugin image preview
        FilePond.registerPlugin(FilePondPluginImagePreview);
        // register plugin file validate type
        FilePond.registerPlugin(FilePondPluginFileValidateType);
        // register plugin file validate size
        FilePond.registerPlugin(FilePondPluginFileValidateSize);
        // register plugin image transform
        FilePond.registerPlugin(FilePondPluginImageTransform);
        // register plugin image resize
